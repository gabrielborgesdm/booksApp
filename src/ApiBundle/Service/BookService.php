...
class BookService extends BaseService
{
  ...
  public function getJson($entity){
    ...
    if(is_array($entity)){
      ...
      $array = $this->pushJson($entity);
      return $array;
    }

  }

  private function pushJson($entity){

    $array["bookName"] = $entity->getBookName();
    $array["bookId"] = $entity->getId();
    $array["pages"] = $entity->getPages();
    $array["publishingDate"] =  $entity->getPublishingDate()->format('F d, Y');
    $array["publisher"] = $entity->getPublisher()->getPublisherName();
    $array["edition"] = $entity->getEdition();
    $array["category"] = $entity->getCategory();
    $array["author"] = "";
    for($i = 0; $i < count($entity->getAuthors()); $i++) {
      $array["author"] .= $entity->getAuthors()[$i]->getAuthorName() . ", ";
